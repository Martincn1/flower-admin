...
export default ({ modifyHandler, changeStatus }) => ColumnFieldClass.generateField([
  ...
  { type: 'base', prop: 'name', label: '用户名', minWidth: 100, formatter: (row) => formatNormalize(row.name) },
  ...
  { type: 'base', prop: 'phone', label: '手机号', minWidth: 120, formatter: (row) => formatNormalize(row.phone) },
  { type: 'base', prop: 'integral', label: '积分', minWidth: 80, formatter: (row) => formatNormalize(row.integral) },
  ...
  { type: 'base', prop: 'endAt', label: '课程有效期', minWidth: 160, formatter: (row) => formatNormalize(row.endAt) },
  { type: 'base', prop: 'grade', label: '所属年级', minWidth: 120, formatter: (row) => formatNormalize(row.grade) },
