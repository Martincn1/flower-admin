...
import { getPrizeList, addPrizeInfo, updatePrizeInfo, deletePrizeInfo } from 'api/lottery-manage'
...
export default {
  ...
  methods: {
    async changeStatus(val, row) {
      const { id } = row
      const { _success } = await updatePrizeInfo({ id, status: val })
      if (!_success) return
      this.fetchData()
    ...
    async deleteHandler(row) {
      ...
      if (!confirm) return
      const { id } = row
      const { _success } = await deletePrizeInfo({ id })
      if (!_success) return
      this.fetchData()
    ...
    async fetchData() {
      ...
      const { _success, data } = await getPrizeList(params)
      if (!_success) return
      this.list = data.data
